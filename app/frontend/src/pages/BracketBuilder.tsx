...
export const BracketBuilder: React.FC = () => {
  ...
  // Renders an outer column of 3rd-place selectors aligned to the given R32 slots.
  // Odd R32 slots (certain seedings) get an invisible placeholder to preserve spacing.
  const renderThirdPlaceSelectorColumn = (slots: string[]) => {
    const locked = tournamentStarted || !isEditing || !!bracket?.is_locked;
    return slots.map(s => {
      const tpConfig = R32_THIRD_PLACE_SLOTS.find(c => c.slot === s);
      return (
        <div key={s} className="flex items-center">
          {tpConfig ? (
            <ThirdPlaceSelector
              ...
              groupPicks={groupPicks}
              logos={logos}
              selected={thirdPlaceSelections[s] ?? null}
              alreadyUsed={Object.entries(thirdPlaceSelections)
                .filter(([k]) => k !== s)
                .map(([, t]) => t)}
              onSelect={(team) => setThirdPlaceSelections(prev => {
                const next: Record<string, string> = {};
                Object.entries(prev).forEach(([sk, t]) => { if (t !== team) next[sk] = t; });
                next[s] = team;
                return next;
              })}
              locked={locked}
  ...
  const koBreakdown = bracket?.points_breakdown?.ko_stages ?? {};
  const KO_STAGE_KEYS: Record<string, string> = {
    'R32': 'round_32',
    'R16': 'round_16',
    'QF': 'quarter_final',
    'SF': 'semi_final',
  ...
  const renderStageLabel = (label: string) => {
    const key = KO_STAGE_KEYS[label];
    const pts = key ? koBreakdown[key] : undefined;
    return (
      <div className="flex flex-col items-center gap-0.5 px-1">
        <span className="text-[9px] text-white/20 uppercase font-bold tracking-wider">{label}</span>
        {pts != null && pts > 0 && (
          <span className="text-[8px] font-bold text-amber-400/80 tabular-nums">+{pts}</span>
        )}
      </div>
    );
  };
          ...
          <div className="glass-card rounded-2xl border border-white/5 overflow-hidden">
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="overflow-x-auto">
                <div className="min-w-max flex items-stretch gap-1.5 min-h-[700px]">

                  {/* Left Wing: 3rd-place selectors | R32(1-8) | R16(1-4) | QF(1-2) | SF-1 */}
                  <div className="flex gap-1.5 flex-1 items-stretch">
                    {/* Outer-left: 3rd-place selectors for R32-2,4,6,8 */}
                    <div className="flex flex-col w-28 shrink-0">
                      {renderStageLabel('3rd pick')}
                      <div className="flex flex-col justify-around gap-1 flex-1">
                        {renderThirdPlaceSelectorColumn(['R32-1','R32-2','R32-3','R32-4','R32-5','R32-6','R32-7','R32-8'])}
                      </div>
                    </div>
                    <div className="flex flex-col justify-around gap-1 shrink-0">
                      {renderStageLabel('R32')}
                      {['R32-1','R32-2','R32-3','R32-4','R32-5','R32-6','R32-7','R32-8'].map(s => renderMatchCard(s, true, false))}
                    </div>
                    <div className="flex flex-col justify-around gap-1.5 flex-1">
                      {renderStageLabel('R16')}
                      {['R16-1','R16-2','R16-3','R16-4'].map(s => renderMatchCard(s))}
                    </div>
                    <div className="flex flex-col justify-around gap-1.5">
                      {renderStageLabel('QF')}
                      {['QF-1','QF-2'].map(s => renderMatchCard(s))}
                    </div>
                  ...
                  {/* Right Wing: SF-2 | QF(3-4) | R16(5-8) | R32(9-16) | 3rd-place selectors */}
                  <div className="flex gap-1.5 flex-1 items-stretch">
                    <div className="flex flex-col justify-around">
                      {renderStageLabel('SF')}
                      {renderMatchCard('SF-2')}
                    </div>
                    <div className="flex flex-col justify-around gap-1.5">
                      {renderStageLabel('QF')}
                      {['QF-3','QF-4'].map(s => renderMatchCard(s))}
                    </div>
                    <div className="flex flex-col justify-around gap-1.5 flex-1">
                      {renderStageLabel('R16')}
                      {['R16-5','R16-6','R16-7','R16-8'].map(s => renderMatchCard(s))}
                    </div>
                    <div className="flex flex-col justify-around gap-1 shrink-0">
                      {renderStageLabel('R32')}
                      {['R32-9','R32-10','R32-11','R32-12','R32-13','R32-14','R32-15','R32-16'].map(s => renderMatchCard(s, true, false))}
                    </div>
                    {/* Outer-right: 3rd-place selectors for R32-10,12,14,16 */}
                    <div className="flex flex-col w-28 shrink-0">
                      {renderStageLabel('3rd pick')}
                      <div className="flex flex-col justify-around gap-1 flex-1">
                        {renderThirdPlaceSelectorColumn(['R32-9','R32-10','R32-11','R32-12','R32-13','R32-14','R32-15','R32-16'])}
                      </div>
                    </div>
                  </div>
              ...
              <div className="overflow-x-auto">
                <div className="min-w-max flex items-stretch gap-1.5 min-h-[700px]">
                  ...
                  <div className="flex gap-1.5 flex-1 items-stretch">
                    <div className="flex flex-col justify-around gap-1 shrink-0">
                      {renderStageLabel('R32')}
                      {['R32-1','R32-2','R32-3','R32-4','R32-5','R32-6','R32-7','R32-8'].map(s => renderActualMatchCard(s, true))}
                    </div>
                    <div className="flex flex-col justify-around gap-1.5 flex-1">
                      {renderStageLabel('R16')}
                      {['R16-1','R16-2','R16-3','R16-4'].map(s => renderActualMatchCard(s))}
                    </div>
                    <div className="flex flex-col justify-around gap-1.5">
                      {renderStageLabel('QF')}
                      {['QF-1','QF-2'].map(s => renderActualMatchCard(s))}
                    </div>
                    <div className="flex flex-col justify-around">
                      {renderStageLabel('SF')}
                      {renderActualMatchCard('SF-1')}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-1.5 flex-1 items-stretch">
                    <div className="flex flex-col justify-around">
                      {renderStageLabel('SF')}
                      {renderActualMatchCard('SF-2')}
                    </div>
                    <div className="flex flex-col justify-around gap-1.5">
                      {renderStageLabel('QF')}
                      {['QF-3','QF-4'].map(s => renderActualMatchCard(s))}
                    </div>
                    <div className="flex flex-col justify-around gap-1.5 flex-1">
                      {renderStageLabel('R16')}
                      {['R16-5','R16-6','R16-7','R16-8'].map(s => renderActualMatchCard(s))}
                    </div>
                    <div className="flex flex-col justify-around gap-1 shrink-0">
                      {renderStageLabel('R32')}
                      {['R32-9','R32-10','R32-11','R32-12','R32-13','R32-14','R32-15','R32-16'].map(s => renderActualMatchCard(s, true))}
                    </div>
                  </div>
              ...
              <p className="text-xs text-white/20 text-center">
                Odd R32 slots: seeded from group picks · Even R32 slots: group winner vs your predicted 3rd-place team (use the outer "3rd pick" columns)
              </p>
            </div>
          </div>
        )
