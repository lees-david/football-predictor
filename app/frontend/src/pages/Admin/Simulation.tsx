...
const FixtureRow: React.FC<FixtureRowProps> = ({ fixture, onSetResult, onReset, isPending }) => {
  ...
  const inputClass = 'w-10 text-center bg-slate-800 border border-white/10 rounded px-1 py-1 text-white text-sm focus:outline-none focus:border-primary disabled:opacity-30 disabled:cursor-not-allowed';
  ...
  return (
    <div className="flex items-center gap-2 py-2 px-3 rounded-lg hover:bg-white/5 transition-colors flex-wrap">
      {/* Teams + FT score */}
      <div className="flex items-center gap-1.5 flex-1 min-w-0">
        <span className="text-sm text-textMain truncate text-right w-20">{fixture.home_team}</span>
        <div className="flex flex-col items-center gap-0.5">
          ...
          <div className="flex items-center gap-1">
            <input type="number" min={0} max={20} value={home} onChange={(e) => setHome(e.target.value)}
              className={inputClass} disabled={isCompleted} />
        ...
        {isKo && (
          <div className="flex flex-col items-center gap-0.5 ml-2">
            <span className={`text-[9px] uppercase tracking-wider ${ftDraw || isCompleted ? 'text-amber-400/70' : 'text-textMuted/40'}`}>AET</span>
            <div className="flex items-center gap-1">
              <input type="number" min={0} max={20} value={homeAet} onChange={(e) => setHomeAet(e.target.value)}
                placeholder="–" className={inputClass} disabled={isCompleted || !ftDraw} />
              <span className="text-textMuted text-xs">–</span>
        ...
        {isKo && (
          <div className="flex flex-col items-center gap-0.5 ml-2">
            <span className={`text-[9px] uppercase tracking-wider ${aetDraw || isCompleted ? 'text-blue-400/70' : 'text-textMuted/40'}`}>Pens</span>
            <select
              value={pensWinner}
              onChange={(e) => setPensWinner(e.target.value as 'home' | 'away' | '')}
              disabled={isCompleted || !aetDraw}
              className="bg-slate-800 border border-white/10 rounded px-1.5 py-1 text-white text-xs focus:outline-none focus:border-primary disabled:opacity-30 disabled:cursor-not-allowed w-20"
            >
              <option value="">–</option>
        ...
        <span className="text-sm text-textMain truncate w-20 ml-1.5">{fixture.away_team}</span>
      </div>

      <Badge variant={statusVariant(fixture.status)} className="w-24 justify-center text-xs shrink-0">{fixture.status}</Badge>
      <div className="flex gap-1.5 shrink-0">
        {!isCompleted ? (
          <Button size="sm" variant="primary" onClick={handleSet}
            disabled={isPending} className="flex items-center gap-1 text-xs">
            <Play size={11} /> Set
          </Button>
        ) : (
          <Button size="sm" variant="secondary" onClick={() => onReset(fixture.id)}
            disabled={isPending} className="flex items-center gap-1 text-xs text-textMuted hover:text-white">
