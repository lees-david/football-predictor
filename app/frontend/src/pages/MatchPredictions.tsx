...
export const MatchPredictions: React.FC = () => {
  ...
  const renderFixtureCard = (fixture: typeof sortedFixtures[0]) => {
    ...
    return (
      <Card key={fixture.id} className={`relative flex flex-col justify-between p-5 min-h-[220px] transition-all duration-300 rounded-2xl glass-card ${cardClasses}`}>
        ...
        <div className="flex items-center justify-between gap-3 my-auto py-2">
          ...
          <div className="flex-1 flex flex-col items-center gap-2 text-center">
            {fixture.home_logo ? (
              <img
                ...
                alt={cleanHome}
                className="w-8 h-5 object-cover rounded shadow-md border border-white/10 filter drop-shadow"
              />
            ) : (
              <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center font-bold text-lg text-white">
                {cleanHome[0] || '?'}
              </div>
            )}
            <span className="text-sm font-semibold text-white/90 truncate w-full max-w-[100px]" title={cleanHome}>
              {cleanHome}
              {getFifaRanking(cleanHome) && (
                <span className="text-[10px] text-textMuted font-mono font-normal ml-1 opacity-70">
          ...
          {/* Mid Section (Prediction vs Result) */}
          <div className="flex flex-col items-center gap-1.5 px-2">

            {(state === 'live' || state === 'completed') && (
              <div className="flex flex-col items-center gap-0.5 mb-1">
                <div className="text-2xl font-black text-white tracking-widest bg-white/5 px-3 py-1 rounded-xl whitespace-nowrap">
                  {fixture.home_score} : {fixture.away_score}
                </div>
                {fixture.home_score_aet !== null && fixture.away_score_aet !== null && (
                  <span className="text-[10px] text-amber-400 font-semibold uppercase tracking-wide whitespace-nowrap">
                    AET {fixture.home_score_aet} : {fixture.away_score_aet}
                  </span>
                )}
                {fixture.knockout_winner && fixture.home_score_aet === fixture.away_score_aet && (
                  <span className="text-[10px] text-violet-400 font-semibold uppercase tracking-wide whitespace-nowrap">
            ...
            <div className="flex items-center gap-1.5">
              {inputsDisabled ? (
                <div className="flex flex-col items-center">
                  <span className="text-xs text-textMuted uppercase tracking-widest font-semibold mb-0.5 whitespace-nowrap">Your Pick</span>
                  {savedPick ? (
                    <div className="text-lg font-bold text-amber-400 bg-amber-500/10 border border-amber-500/20 px-3 py-0.5 rounded-lg whitespace-nowrap">
                      {savedPick.predicted_home} : {savedPick.predicted_away}
                    </div>
                  ) : (
                    <span className="text-xs text-textMuted italic bg-white/5 px-2.5 py-1 rounded-lg">No prediction</span>
                  )}
                </div>
              ) : (
                <>
                  <input
                    ...
                    className="w-11 h-11 text-center text-xl font-bold bg-black/40 border border-white/10 rounded-xl focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none transition-all disabled:opacity-50"
                  />
          ...
          <div className="flex-1 flex flex-col items-center gap-2 text-center">
            ...
            <span className="text-sm font-semibold text-white/90 truncate w-full max-w-[100px]" title={cleanAway}>
              {getFifaRanking(cleanAway) && (
                <span className="text-[10px] text-textMuted font-mono font-normal mr-1 opacity-70">
        ...
        <div className="flex justify-between items-center mt-3 pt-3 border-t border-t-white/5">
          ...
          <div className="text-xs">
            {savedPick && !hasUnsavedChanges ? (
              ...
              <span className="text-amber-400 font-semibold animate-pulse">Unsaved changes</span>
            ) : null}
          </div>
        </div>
