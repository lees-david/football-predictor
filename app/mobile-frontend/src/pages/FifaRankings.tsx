...
import { Search, Trophy, ArrowRight, ShieldAlert } from 'lucide-react';
      ...
      {teams.length === 0 ? (
        ...
        <div className="space-y-4 animate-fadeIn">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex justify-between items-center text-xs text-textMuted font-semibold">
              <div className="flex items-center gap-2">
                <span>Sort by:</span>
                <button
                  onClick={() => handleSort('rank')}
                  className={`px-2 py-1 rounded bg-white/5 border border-white/5 text-[11px] font-bold transition-all ${
                    sortField === 'rank' ? 'text-amber-400 border-amber-500/30 bg-amber-500/5' : ''
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-2 py-1 rounded bg-white/5 border border-white/5 text-[11px] font-bold transition-all ${
                    sortField === 'name' ? 'text-amber-400 border-amber-500/30 bg-amber-500/5' : ''
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-2 py-1 rounded bg-white/5 border border-white/5 text-[11px] font-bold transition-all ${
                    sortField === 'group' ? 'text-amber-400 border-amber-500/30 bg-amber-500/5' : ''
                  }`}
                >
                  Group {sortField === 'group' && (sortAsc ? '↑' : '↓')}
                </button>
              </div>
              <div className="shrink-0 text-[10px] uppercase tracking-wider text-textMuted/80">
          ...
          <div className="space-y-3">
            {filteredAndSortedTeams.map(t => (
              <div
                ...
                className={`p-4 rounded-xl border border-white/5 bg-black/35 hover:border-white/15 transition-all duration-200 flex items-center justify-between group cursor-pointer ${
                  t.isEliminated ? 'opacity-70 bg-black/10' : ''
                }`}
              >
                <div className="flex items-center gap-3.5 min-w-0">
                  <span className="text-sm font-mono font-bold text-amber-500 w-8 shrink-0">
                    ...
                    <div className="flex items-center gap-2.5">
                      {t.logo ? (
                        <img
                          src={t.logo}
                          alt={t.name}
                          className="w-7 h-4.5 object-cover rounded shadow-sm border border-white/10 shrink-0"
                        />
                      ) : (
                        <div className="w-7 h-5 rounded bg-white/5 flex items-center justify-center font-bold text-xs text-white shrink-0">
                          {t.name[0]}
                        </div>
                      )}
                      <span className={`font-semibold text-white group-hover:text-amber-400 transition-colors truncate ${t.isEliminated ? 'text-textMuted' : ''}`}>
                        {t.name}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] text-textMuted mt-1 flex-wrap">
                      <span>Group {t.groupCode}</span>
                      <span>•</span>
                      <span>FIFA: {t.points ?? '—'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="shrink-0 ml-2">
                  {t.isEliminated ? (
                    <span className="text-[10px] text-red-500/80 font-bold uppercase tracking-wider bg-red-500/10 px-2 py-0.5 rounded-full border border-red-500/20">
                      Eliminated
                    </span>
                  ) : (
                    <Button
                      ...
                      className="inline-flex items-center gap-1 bg-amber-500/10 hover:bg-amber-500 text-amber-400 hover:text-black border border-amber-500/20 hover:border-amber-500 rounded-lg text-[10px] py-1 px-2.5 transition-all duration-200"
                    >
                      Predict <ArrowRight size={10} />
                    </Button>
                  )}
                </div>
              </div>
